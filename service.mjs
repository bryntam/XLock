...
function publicState() {
  return {
    ...state,
    twitterAllowed: state.armed ? state.twitterAllowed : true,
    lockActive: state.armed,
    elapsedSeconds: elapsedSeconds()
  };
}

async function startSession(source = "manual") {
  if (!state.armed) return publicState();
...
<style>
  ...
  .arm{background:#36c275;color:#06140c}.end{background:#ff6b57;color:#1b0905}.plain{background:#2b303a;color:#f5f5f0}
...
<main>
  ...
  <button class="arm" onclick="post('/gate/arm')">Arm XLock</button>
  <button class="plain" onclick="post('/gate/pause')">Pause XLock</button>
...
async function refresh(){
  ...
  label.textContent = data.armed ? 'X is' : 'XLock is';
  mode.textContent = data.armed ? (data.twitterAllowed ? 'unlocked' : 'blocked') : 'paused';
  meta.textContent = (data.armed ? data.mode : 'off') + ' · ' + data.sessionSource + ' · ' + data.elapsedSeconds + 's' + (data.autoEndedAt ? ' · auto-locked' : '');
}
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "POST" && url.pathname === "/gate/arm") return send(response, 200, { ok: true, data: await armGate() });
  if (request.method === "POST" && url.pathname === "/gate/pause") return send(response, 200, { ok: true, data: await pauseGate() });
